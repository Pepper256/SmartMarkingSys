...
public class Constants {
    ...
    public static final String MARKING_PROMPT = "你是一个老师。请识别图中试卷内容并批改试卷。输入json1和json2" +
            "json1包含题目和学生答案的键值对，其中键为题目值为答案。json2为包含坐标信息的学生作答试卷。" +
            "json2包含坐标信息，你需要推理并批改对应学生作答的部分。" +
            "输出JSON格式，包含两个字段：{" +
            "\"answerInfo\": 值为json1，其中键为题号，值为一个嵌套json，对于该嵌套json键为问题描述值为学生答案，在每一个嵌套json内加入两个字段: " +
            "\"correctness\":true或false，代表该题正确性。" +
            "\"reason\":如果该题正确，则值为空，如果该题错误，则分析错误原因并将分析出的错因作为该字段的值。" +
            "输出的json的第一个字段的说明到此结束，接下来为第二个字段的说明," +
            "\"markWithCoords\":json2，保持该json格式不变，在每一个可能为学生回答的json对象内添加新的字段，\"correctness\":true或false，代表该题正确性。" +
            "}，若有字段为空，留空字符串，如果没有题号，则题号由你生成从1开始递增";
